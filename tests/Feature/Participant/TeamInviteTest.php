...
class TeamInviteTest extends TestCase
{
    ...
    public function test_pending_invites_cannot_overflow_the_team(): void
    {
        ...
        // Sobra UMA vaga. A checagem de lotacao roda no envio, e nada foi
        // aceito ainda, entao o lider consegue mandar dois convites.
        $primeiro = $this->inscrito($event);
        $segundo = $this->inscrito($event);

        $conviteA = TeamInvite::factory()->for($event)->for($team)->create(['email' => $primeiro->email]);
        $conviteB = TeamInvite::factory()->for($event)->for($team)->create(['email' => $segundo->email]);

        $this->actingAs($primeiro)->get(route('team-invites.accept', $conviteA->token));
        $this->actingAs($segundo)->get(route('team-invites.accept', $conviteB->token));

        $this->assertSame(
            2,
            $team->fresh()->activeMemberCount(),
            'A equipe nao pode ultrapassar max_team_size por convites pendentes.'
        );
    }
}
